...
subfolders = [
    folder
    for folder in os.listdir(PROFILE_PICTURES_DIRECTORY)
    if os.path.isdir(os.path.join(PROFILE_PICTURES_DIRECTORY, folder))
]
session = Session()
...
def json_db_status_synced():
    ...
    stmt = select(
        users_table.contact_name, status_table.status, status_table.timestamp
    ).join(status_table)
...
def json_db_picture_synced() -> bool:
    ...
    stmt = select(
        users_table.contact_name,
        pictures_table.picture_filename,
        pictures_table.timestamp,
    ).join(pictures_table)
    ...
    for contact_name in contact_names:
        ...
        jpg_files = [
            "_".join(name.split("_")[1:]).split(".")[0]
            for name in os.listdir(user.userdir)
    ...
    not_saved = {
        (name, picture_filename) for (name, picture_filename, timestamp) in db_pictures
    } - saved_pictures
    if not not_in_db == not_in_js == set():
        print("ERROR: js and db pictures are not synced")
    ...
    if not_saved != set():
        print("ERROR: saved images and db pictures are not synced")
        print(f"pictures in db but not saved: {not_saved}")
        return False
    print("all images in db and json are saved")
    return True
